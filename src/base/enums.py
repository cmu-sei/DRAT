...
class FileOrigin(Enum):
    """
    Defines the origin of a file on the system
    """
    OSInstalled = auto()
    PackageInstalled = auto()
    PackageModified = auto()  # Grab
    PackageContent = auto()  # Grab
    OtherExecutable = auto()  # Grab
    UserData = auto()  # Grab
    UnknownSource = auto()
    EphemeralContent = auto()


class OSDistro(Enum):
    """
    OS Distros that we can process
    """
    CentOS = auto()
    Other = auto()
